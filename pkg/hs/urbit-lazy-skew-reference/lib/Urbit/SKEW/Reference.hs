...
data SKEW = S | K | E | W | Q | J (Int, Expr, Expr) deriving (Eq, Ord, Show)
...
nf :: Expr -> Maybe Expr
nf (whnf -> Just x)      = Just x
nf ((nf -> Just x) :@ y) = Just (x :@ y)
nf (x :@ (nf -> Just y)) = Just (x :@ y)
nf _                     = Nothing
...
whnf = \case
  ...
  (tree -> Node (J(n,t,b)) xs)              | t /= N E && length xs == n
                                           -> Just (foldl' (:@) b (expr <$> xs))
  ...
  N W :@a:@s:@k:@e:@w:@q :@ N (J(n,t,b))   -> Just (a :@ N (J(n-1,N E,t)) :@ b)
